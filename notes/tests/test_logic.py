...
from .confunittest import NotesUrls, NOTE_SLUG
...
class TestLogic(NotesUrls):
    ...
    def test_reader_can_create_notes_slug(self):
        ...
        notes.delete()
        notes_count_start_test = Note.objects.count()
        self.author_client.post(self.add_url, data=self.form_data_new)
        notes_count = Note.objects.count()
        self.assertEqual(notes_count, notes_count_start_test + 1)
        new_note = Note.objects.get()
        expected_slug = slugify(self.form_data_new['title'])
